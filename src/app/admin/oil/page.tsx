...
function OilTable({ lines, options }: { lines: OilRow[]; options: Option[] }) {
  return (
...
export default async function OilPage({
  ...
  const [buckets, dayLines, unassignedLines] = await Promise.all([
    prisma.oilBucket.findMany({
      orderBy: [{ active: "desc" }, { name: "asc" }],
      include: { _count: { select: { oilLines: true } } },
    }),
    prisma.oilLine.findMany({
  ...
  return (
    <div className="space-y-4 pb-6">
      <MasterListManager
        title="Oil buckets"
        endpoint="/api/oil-buckets"
        noun="oil type"
        items={buckets.map((b) => ({
          id: b.id,
          name: b.name,
          active: b.active,
          count: b._count.oilLines,
        }))}
      />
      ...
      <section className="rounded-xl bg-surface p-4 shadow-soft ring-1 ring-border">
        ...
        {unassignedLines.length === 0 ? (
          <p className="py-4 text-center text-sm text-faint">Nothing waiting — all oil entries are classified.</p>
        ) : (
          <OilTable lines={unassignedLines} options={options} />
        )}
      </section>
      ...
      <section className="rounded-xl bg-surface p-4 shadow-soft ring-1 ring-border">
        ...
        {dayLines.length === 0 ? (
          <p className="py-6 text-center text-sm text-faint">No oil entries on {dayLabel(date)}.</p>
        ) : (
          <OilTable lines={dayLines} options={options} />
        )}
      </section>
    </div>
  );
